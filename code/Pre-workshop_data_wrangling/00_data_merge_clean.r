# Data wrangling to prep for workshop (Intersession spring 26)
...
summary(d2010) # all variables but Year, Rating, and méta_score are character
...
movies_cleaned <- movies_cleaned |>
  mutate(votes = case_when(
    grepl("M", Votes, ignore.case = TRUE) ~ as.numeric(gsub("[Mm]", "", Votes)) * 1e6,
    grepl("K", Votes, ignore.case = TRUE) ~ as.numeric(gsub("[Kk]", "", Votes)) * 1e3,
    TRUE                                   ~ as.numeric(Votes)
  ))
...
write_csv(movies_subset_dollars, "data/movie_data.csv")
